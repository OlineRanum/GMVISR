...
class PonitaFiberBundle(nn.Module):
    ...
    def __init__(self,
                 ...
                 layer_scale=None,
                 task_level='graph',
                 multiple_readouts=True, n_targets =2,
                 **kwargs):
        super().__init__()
        ...
        self.read_out_layers_hs = nn.ModuleList()
        ...
        for i in range(num_layers):
            ...
            if n_targets == 1:
                ...
                self.read_out_layers_hs.append(None)
            elif n_targets == 2:
                self.read_out_layers.append(nn.Linear(hidden_dim, output_dim + output_dim_vec))
                self.read_out_layers_hs.append(nn.Linear(hidden_dim, output_dim + output_dim_vec))
            else:
